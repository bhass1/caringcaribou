            ...
            if(response_service_id == expected_response_id
               and subfunction == reset_type):
                ...
                if response_length > 2:
                    ...
                    print("Response contains additional data: [{0}]"
...
def __parse_args(args):
    ...
    parser = argparse.ArgumentParser(
                    ...
                    description="""Universal Diagnostic Services module
                        for CaringCaribou""",
    ...
    parser_ecu_reset.add_argument("src", type=parse_int_dec_or_hex,
                                  help="arbitration ID to transmit to")
    parser_ecu_reset.add_argument("dst", type=parse_int_dec_or_hex,
                                  help="arbitration ID to listen to")
    parser_ecu_reset.add_argument("-t", "--timeout", type=float,
                                  metavar="T",
                                  help="wait T seconds for response before "
                                  "timeout")
